...
public static partial class Filtered
{
	...
	public class Events : Filtered<EventInfo, Events>, IDescribableSubject
	{
		...
		/// <summary>
		///     A Container for a filterable collection of <see cref="EventInfo" />,
		///     that also allows specifying string equality options.
		/// </summary>
		public class StringEqualityResult : Events
		{
			private readonly StringEqualityOptions _options;

			internal StringEqualityResult(Events inner, StringEqualityOptions options) : base(inner)
			{
				_options = options;
			}
		...
		public class StringEqualityResultType : StringEqualityResult
		{
			...
			internal StringEqualityResultType(Events inner, StringEqualityOptions options) : base(inner, options)
			{
				_options = options;
			}
