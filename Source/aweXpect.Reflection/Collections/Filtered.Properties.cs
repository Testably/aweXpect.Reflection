...
public static partial class Filtered
{
	...
	public class Properties : Filtered<PropertyInfo, Properties>, IDescribableSubject
	{
		...
		/// <summary>
		///     A Container for a filterable collection of <see cref="PropertyInfo" />,
		///     that also allows specifying string equality options.
		/// </summary>
		public class StringEqualityResult : Properties
		{
			private readonly StringEqualityOptions _options;

			internal StringEqualityResult(Properties inner, StringEqualityOptions options) : base(inner)
			{
				_options = options;
			}
		...
		public class StringEqualityResultType : StringEqualityResult
		{
			...
			internal StringEqualityResultType(Properties inner, StringEqualityOptions options) : base(inner, options)
			{
				_options = options;
			}
